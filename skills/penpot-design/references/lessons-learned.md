...
3. Classifique como `kind: machine` (somente memória da estação) ou
   `kind: project` (regra integrada ao workflow). Lições `project` exigem uma
   referência explícita de integração.
