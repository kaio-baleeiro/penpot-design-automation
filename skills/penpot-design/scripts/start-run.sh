...
cd "$project_root"
for argument in "$@"; do [[ "$argument" != --help && "$argument" != -h ]] || exec "$python_bin" -m scripts.penpot_validation start-run "$@"; done
"$python_bin" "$project_root/scripts/workflow_guard.py" require design >/dev/null
exec "$python_bin" -m scripts.penpot_validation start-run "$@"
